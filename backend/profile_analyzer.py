...
def fit_and_plot_profiles(explist, exptitles, method='mean', col_nums=2, profile_axis='x', fit_function='gauss', num_xticks=5, num_yticks=5):
    ...
    for i, (df, title) in enumerate(zip(explist, exptitles)):
        ...
        try:
            ...
            ax.plot(profile.index, fit_func(x_data, *popt), 'r--', label=f'Fit: x0={profile.index[int(round(popt[1]))]:.2f}, FWHM={fwhm_value:.5f}')

            fwhm_start = profile.index[int(round(popt[1]))] - fwhm_value / 2
            fwhm_end = profile.index[int(round(popt[1]))] + fwhm_value / 2
